...
public class SystemLogService extends BaseService<SystemLog> {
	 ...
	 * @return
	 */
	public Page<SystemLog> paginateSystemLog(Integer pageNumber,Integer pageSize,String keywords, Date startTime, Date endTime) {
		Kv paras=Kv.create();
		...
		if(isOk(startTime)){
			paras.set("createTime >=",DateUtil.HHmmssTo000000Str(startTime));
		}
		if(isOk(endTime)){
			paras.set("createTime <=",DateUtil.HHmmssTo235959Str(endTime));
		}
	 ...
	 */
	public void saveLog(int type,int targetType,int targetId,String title,int dictionaryType,Integer userId,String userName){
