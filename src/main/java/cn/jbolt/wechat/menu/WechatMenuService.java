...
public class WechatMenuService extends BaseService<WechatMenu> {
	...
	public List<WechatMenu> getLevel1Menus(Integer mpId) {
		return getCommonList(Kv.by("mpId", mpId).set("pid",0),"sortRank");
	}
	/**
	 * 得到二级菜单
	 * @param mpId
	...
	public List<WechatMenu> getLevel2Menus(Integer mpId,Integer pid) {
		return getCommonList(Kv.by("mpId", mpId).set("pid",pid),"sortRank");
	}
	/**
	 * 得到子菜单
	 * @param mpId
	...
	public List<WechatMenu> getListByPid(Integer mpId,Integer pid) {
		return getCommonList(Kv.by("mpId", mpId).set("pid",pid==null?0:pid),"sortRank");
	}
	/**
	 * 保存
	 * @param userId
	...
	public Ret save(Integer userId, Integer mpId, WechatMenu menu) {
		...
		menu.setMpId(mpId);
		int rank=getNextSortRank(Kv.by("mpId",mpId).set("pid", menu.getPid()));
		...
		return success?success(menu,Msg.SUCCESS):fail(Msg.FAIL);
	}
	/**
	 * 更新
	 * @param userId
	...
	public Ret update(Integer userId, Integer mpId, WechatMenu menu) {
		...
		menu.setMpId(mpId);
		if(oldRank!=newRank){
			WechatMenu otherMenu=findFirst(Kv.by("mpId =",mpId).set("pid =", db.getPid()).set("sortRank =", newRank).set("id !=", db.getId()),true);
			if(otherMenu!=null){
		...
		return success?success(db,Msg.SUCCESS):fail(Msg.FAIL);
	}
	/**
	 * 删除
	 * @param userId
	...
	private int getCountByPid(Integer mpId, Integer pid) {
		return getCount(Kv.by("mpId", mpId).set("pid",pid));
	}
	/**
	 * 发布自定义菜单
	 * @param userId
	...
	public Ret publish(Integer userId, Integer mpId) {
		...
		msg.setLength(msg.length()-1);
		msg.append("]}");
		String appId=wechatConfigService.getWechatConfigAppId(mpId);
