...
public class GoodsBackCategoryService extends BaseService<GoodsBackCategory> {
	...
	public List<GoodsBackCategory> getCategorysByPid(Integer pid,Boolean enable) {
		...
		return getCommonList(kv,"sortRank","asc");
	}	
	/**
	 * 根据PID获取分类数据 后端启用的数据
	 * @param pid
	...
	public Ret update(Integer userId, GoodsBackCategory category) {
		...
		if(success){
			Ret ret=processCategoryKey(category);
			if(ret.isOk()){
				CACHE.me.removeGoodsBackCategory(category.getId());
				//添加日志
	...
	public Ret delete(Integer userId, Integer id) {
		...
		if(ret.isOk()){
			CACHE.me.removeGoodsBackCategory(id);
			//添加日志
	...
	public Ret move(Integer userId,Integer id, Integer newPid, Integer newRank) {
		...
		if(oldPid.intValue()==newPid.intValue()){
			Kv paras=Kv.by("pid =", oldPid).set("rank >=", newRank).set("id <>", id);
			List<GoodsBackCategory> others=getCommonList("*", paras, "sortRank", "asc", true);
			//如果没有改变父亲 只调整了顺序
			...
			List<GoodsBackCategory> all=getCommonList("*", allparas, "sortRank", "asc", true);
			doUpdateRanks(all);
			//2、新的里面从新位置往后排列
			//新位置右侧调整顺序
			Kv otherparas=Kv.by("pid =", newPid).set("rank >=", newRank).set("id <>", id);
			List<GoodsBackCategory> others=getCommonList("*", otherparas, "sortRank", "asc", true);
			doUpdateRightRanks(others,newRank+1);
		}
		//最后更细自己
		old.setPid(newPid);
		...
		//缓存
		CACHE.me.removeGoodsBackCategory(id);
		CACHE.me.removeGoodsBackCategory(old.getId());
		//处理Key
	...
	private void processCategoryKeyWithLeafNode(Integer id) {
		...
		for(GoodsBackCategory son:sons){
			...
			//缓存
			CACHE.me.removeGoodsBackCategory(son.getId());
		}
		//缓存
		CACHE.me.removeGoodsBackCategory(id);
	}
	...
	private void doUpdateRanks(List<GoodsBackCategory> others) {
		...
		for(int i=0;i<others.size();i++){
			...
			//缓存
			CACHE.me.removeGoodsBackCategory(others.get(i).getId());
		}
