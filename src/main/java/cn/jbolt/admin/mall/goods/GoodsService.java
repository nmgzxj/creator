...
public class GoodsService extends BaseService<Goods> {
	 ...
	 * @return
	 */
	public Page<Goods> paginateAdminData(Integer pageNumber, int pageSize, String keywords,Boolean onSale,Boolean isHot,Boolean isRecommend,Boolean isDelete,Integer bcategoryId,Integer fcategoryId) {
		...
		return dao().paginate(pageNumber, pageSize, Db.getSqlPara("mall.goods.paginateAdminList", paras));
	}
	/**
...
/*	*//**
	...
	public Ret toggleOnSale(Integer userId, Integer id) {
		Ret ret=toggleBoolean(Kv.by("userId", userId),id, "onSale");
		if(ret.isOk()){
	...
	public Ret toggleIsHot(Integer userId, Integer id) {
		return toggleBoolean(Kv.by("userId", userId),id, "isHot");
	}
	/**
	 * 切换推荐
	 * @param userId
	...
	public Ret toggleIsRecommend(Integer userId, Integer id) {
		return toggleBoolean(Kv.by("userId", userId),id, "isRecommend");
	}
	@Override
	public String checkCanToggle(Goods goods, String column) {
		if(goods.getIsDelete()!=null&&goods.getIsDelete()){
			return "已经进入回收站的商品，不能切换此属性";
		}
		if(column.equals("isRecommend")||column.equals("isHot")){
	...
	public int getDeleteCount() {
		return getCount(Kv.by("isDelete", true));
	}
	...
	public boolean checkGoodsBackCategoryInUse(Integer id) {
		Integer goodsId=Db.queryInt(Db.getSqlPara("mall.goods.checkGoodsBackCategoryInUse",Kv.by("table", table()).set("bcategoryId",id)).getSql());
		return isOk(goodsId);
	}
	/**
	 * 更新营销信息
	...
	private void removeGoodsSpecAttr(Goods goods) {
		goods.remove("bcategoryId","fcategoryId","bcategoryKey","fcategoryKey","typeId");
	}
	/**
	 * 检测是否可以修改category
	 * @param goods
