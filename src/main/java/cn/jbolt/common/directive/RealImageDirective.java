...
import cn.jbolt.common.util.RealIamgeUtil;
/**
 * 正确输出图片地址的指令
 ...
 */
public class RealImageDirective extends Directive {
	...
	public void exec(Env env, Scope scope, Writer writer) {
		if (paraNum == 0) {
			...
			outputRealIamgeUrl(env, scope, writer);
		}
	}
	/**
	 * 输出空字符
	 ...
	 * @param writer
	 */
	private void outputRealIamgeUrl(Env env,Scope scope, Writer writer) {
		...
		if(defaultValueExpr!=null){
			...
			try {
				writer.write(RealIamgeUtil.get(value, defaultValue));
			} catch (IOException e) {
				e.printStackTrace();
			}
